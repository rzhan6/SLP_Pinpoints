...
def getImg_dsPM(dsFd=r'\mnt\DADES2\SLP\SLP\danaLab', idx_subj=1, modality='RGB', cov='uncover',
                idx_frm=1):
	'''
...
def genPTr_dict(subj_li, mod_li, dsFd=r'\mnt\DADES2\SLP\SLP\danaLab'):
	'''
...
class SLP_RD:  # slp reader
	...
	def __init__(self, opts, phase='train', if_0base=True):
		...
		for i in tqdm(idxs_subj_all, desc='initializing SLP'):
			...
			###CHANGES### 
			if 'danaLab' in dsFd:        # only dana has PM data
